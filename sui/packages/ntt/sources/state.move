module ntt::state {
    ...
    public struct State<phantom T> has key, store {
        ...
        // M Token extensions (minimal - object ID references only)
        earner_global_id: Option<ID>,
    ...
    public fun has_m_token_globals<T>(state: &State<T>): bool {
        option::is_some(&state.earner_global_id)
    }
    ...
    public(package) fun set_m_token_globals<T>(
        ...
        option::fill(&mut state.earner_global_id, earner_global_id);
        option::fill(&mut state.registrar_global_id, registrar_global_id);
        option::fill(&mut state.earner_cap_id, earner_cap_id);
        option::fill(&mut state.registrar_cap_id, registrar_cap_id);
    }

    /// ID accessor functions for M Token objects
    public(package) fun get_earner_global_id<T>(state: &State<T>): Option<ID> {
        state.earner_global_id
    }

    public(package) fun get_registrar_global_id<T>(state: &State<T>): Option<ID> {
        state.registrar_global_id
    }

    public(package) fun get_earner_cap_id<T>(state: &State<T>): Option<ID> {
        state.earner_cap_id
    }
